// App-side copy of shared/src/cro-slides.ts.
//
// web/ deploys to Vercel as a self-contained directory and cannot import the shared workspace
// package — the same boundary web/lib/shared.ts exists for. The CLI drafts the UX and competitor
// slides and needs these rules; this app drafts the analytics and key-insights slides and needs
// exactly the same ones. A slide generated here that broke a rule the CLI enforces would sit in the
// same deck as one that did not.
//
// shared/test/cro-slides.test.ts runs both implementations over the same corpus and asserts they
// agree, which is a stronger guard than comparing their source text.

import type { CroBullet, CroBulletEdit, CroEvidenceItem, CroRejectedBullet, CroSlide, CroStepKey } from "./shared";
