...
describe("the blocked/fail boundary", () => {
  ...
  it("never reports a fail for a state-dependent assertion when nothing was reached", () => {
    ...
    // Nothing may fail here, and the set is asserted empty rather than "small": G1 and G3 used to
    // read the clean capture directly and claim "no privacy policy link was found on the page"
    // about a page that never loaded. An allowance for known offenders is how that comes back.
  ...
  it("never reports a fail when the opt-out model applies and nothing was reached", () => {
    ...
    const failing = [...statuses].filter(([, s]) => s === "fail").map(([id]) => id);
    assert.deepEqual(new Set(failing), new Set());
